...
impl RealtimeLoroProtocol {
    ...
    pub async fn broadcast_update(&self, _change_description: &str) -> Result<()> {
        ...
        tracing::info!("Broadcasting update to {} peers", peers.len());
        ...
        for peer_id in peers {
            if let Some(peer) = self.connection_manager.get_peer(peer_id).await {
                ...
                tracing::info!("Updates for peer {}: {} bytes", peer_id, updates.len());
                ...
                if !updates.is_empty() {
                    ...
                    if let Err(e) = self.connection_manager.send_to_peer(peer_id, message).await {
                        tracing::error!("Failed to send update to peer {}: {}", peer_id, e);
                    } else {
                        tracing::info!("Successfully sent update to peer {}", peer_id);
                    }
                } else {
                    tracing::debug!("No updates to send to peer {}", peer_id);
                }
            }
        }
    ...
    pub async fn handle_protocol_message(&self, from_peer: NodeId, message: ProtocolMessage) -> Result<()> {
        match message {
            ProtocolMessage::UpdateBatch { updates, to_version, .. } => {
                tracing::info!("Received update batch from {}: {} bytes", from_peer, updates.len());
                ...
                tracing::info!("Successfully applied updates from {}", from_peer);
...
impl ProtocolHandler for RealtimeLoroProtocol {
    async fn accept(&self, conn: Connection) -> Result<(), AcceptError> {
        ...
        // Handle the initial sync request
        if let Err(e) = self.handle_peer_message(remote_node_id, conn.clone()).await {
            tracing::error!("Failed to handle sync request from {}: {}", remote_node_id, e);
            return Err(AcceptError::from_err(std::io::Error::new(std::io::ErrorKind::Other, e)));
        }

        // Start continuous message handling for real-time updates
        let protocol = self.clone();
        tokio::spawn(async move {
            if let Err(e) = protocol.handle_continuous_messages(remote_node_id, conn).await {
                tracing::error!("Continuous message handling failed for {}: {}", remote_node_id, e);
            }
        });
...
impl RealtimeLoroProtocol {
    /// Handle continuous messages from a peer for real-time updates
    async fn handle_continuous_messages(&self, peer_id: NodeId, conn: Connection) -> Result<()> {
        loop {
            // Accept new bidirectional streams for ongoing updates
            match conn.accept_bi().await {
                Ok((_send, mut recv)) => {
                    // Receive message
                    match self.receive_message(&mut recv).await {
                        Ok(message) => {
                            if let Err(e) = self.handle_protocol_message(peer_id, message).await {
                                tracing::error!("Failed to handle message from {}: {}", peer_id, e);
                            }
                        }
                        Err(e) => {
                            tracing::debug!("Failed to receive message from {}: {}", peer_id, e);
                            break;
                        }
                    }
                }
        ...
        let _ = self.connection_manager.remove_peer(peer_id).await;
